...
filtered = open(filename, 'wb')
wr = csv.DictWriter(filtered, fieldnames=wanted_columns, delimiter=',',quotechar='"', quoting=csv.QUOTE_NONNUMERIC, extrasaction='ignore')
wr.writeheader()
with open('aggregated.csv') as csvfile:
	next(csvfile) # Skip header
	for row in csv.DictReader(csvfile, fieldnames=columns, delimiter=','):
		...
		row[columns[2]] = float(row[columns[2]])
		row[columns[3]] = float(row[columns[3]])
		row[columns[4]] = float(row[columns[4]])
		row[columns[5]] = float(row[columns[5]])
		row[columns[6]] = float(row[columns[6]])
		row[columns[7]] = float(row[columns[7]])
		row[columns[8]] = float(row[columns[8]])
		row[columns[9]] = float(row[columns[9]])
		row[columns[10]] = float(row[columns[10]])
		row[columns[11]] = float(row[columns[11]])
		row[columns[12]] = float(row[columns[12]])
		row[columns[13]] = float(row[columns[13]])
		row[columns[14]] = float(row[columns[14]])
		row[columns[15]] = float(row[columns[15]])
		row[columns[16]] = float(row[columns[16]])
		row[columns[17]] = float(row[columns[17]])
		row[columns[18]] = float(row[columns[18]])
		row[columns[19]] = float(row[columns[19]])
		row[columns[20]] = float(row[columns[20]])
		row[columns[21]] = float(row[columns[21]])
		row[columns[22]] = float(row[columns[22]])
		row[columns[23]] = float(row[columns[23]])
